...
function ArticleContactForm({ data }) {
  ...
  const { showActivitySpinner, hideActivitySpinner, displayNotification } =
    useFeedbacks();
  ...
  const _submitMessage = async () => {
    ...
    const _onSubmitError = () => {
      ...
      displayNotification(
        "error",
        getString("uhOh"),
        getString("message_sent_error")
      );
    };

    // const success = await emails.sendContactEmail(name, email, contact, message)
    try {
      const response = await fetch(
        `https://nikhilkumar.onrender.com/send-email`,
        {
          method: "POST",
          headers: {
